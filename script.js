...
function updateSoundButton() {
    const label = audioManager.isMuted ? "Sound: Aus" : "Sound: An";
...
function isGameKey(code) {
    return ["ArrowLeft", "ArrowRight", "ArrowUp", "Space", "KeyD", "KeyH"].includes(code);
}
...
function bindTouchButton(buttonId, key) {
    ...
    button.addEventListener("touchstart", (event) => setTouchKey(event, key, true));
    button.addEventListener("touchend", (event) => setTouchKey(event, key, false));
