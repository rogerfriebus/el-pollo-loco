class GameController {
  ...
  canStompEnemy(enemy) {
    ...
    return (
      ...
      this.hasEnoughHorizontalStompOverlap(enemy) &&
      this.isInStompZone(enemy)
    );
  }
  ...
  isInStompZone(enemy) {
    ...
    const enemyBox = enemy.getCollisionBox();
    return (
      this.wasAboveEnemy(previousBox, enemyBox) ||
      this.isInUpperEnemyArea(characterBox, enemyBox)
    );
  }
  ...
  playHurtSoundIfNeeded(oldEnergy) {
    if (this.world.character.energy < oldEnergy)
      this.world.audioManager.playHurt();
  }
   ...
   */
  collectCoin(coin) {
    this.world.level.coins = this.world.level.coins.filter(
      (item) => item !== coin,
    );
  ...
  canThrowBottle() {
    ...
    const cooldownReady =
      Date.now() - this.world.lastThrowTime > this.world.throwCooldown;
    return (
      this.world.keyboard.d &&
      hasBottle &&
      cooldownReady &&
      this.world.isPlayable()
    );
  }
  ...
  createThrowableBottle() {
    const x = this.world.character.otherDirection
  ...
  removeFinishedThrowableObjects() {
    this.world.throwableObjects = this.world.throwableObjects.filter(
      (bottle) => !bottle.isReadyForRemoval(),
    );
  }
